...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for AITT
    ...
    if (p_rewardSnapshot.strDistributionAsset != "AITT") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is AITT with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using AITT differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "AITT") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse AITTCoin address (already validated during ownership snapshot creation)
