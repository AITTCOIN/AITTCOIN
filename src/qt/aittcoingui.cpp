...
#include "aittcoingui.h"

#include "aittcoinunits.h"
...
const std::string AITTCoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString AITTCoinGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "AITTBTC"  , 1,          8},
    { "mBTC",   "AITTBTC"  , 1000,       5},
    { "µBTC",   "AITTBTC"  , 1000000,    2},
    { "Satoshi","AITTBTC"  , 100000000,  0},
    { "USDT",   "AITTUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(AITTCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

AITTCoinGUI::AITTCoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void AITTCoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a AITTCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and aittcoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to AITT addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your AITTCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified AITTCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a aittcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible AITTCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void AITTCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void AITTCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/aitttext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/aittcointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* aittcoinLabelLayout = new QVBoxLayout(toolbarWidget);
        aittcoinLabelLayout->addWidget(labelToolbar);
        aittcoinLabelLayout->addWidget(m_toolbar);
        aittcoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        aittcoinLabelLayout->addStretch(1);
        ...
        // labelCurrentMarket->setText(tr("AITTCOIN Market Price"));
        ...
        comboAITTUnit = new QComboBox(headerWidget);
        QStringList list;
        for(int unitNum = 0; unitNum < CurrencyUnits::count(); unitNum++) {
            list.append(QString(CurrencyUnits::CurrencyOptions[unitNum].Header));
        }
        comboAITTUnit->addItems(list);
        comboAITTUnit->setFixedHeight(26);
        comboAITTUnit->setContentsMargins(5,0,0,0);
        ...
        // priceLayout->addWidget(comboAITTUnit, 0 , Qt::AlignBottom| Qt::AlignLeft);
        ...
        connect(comboAITTUnit, SIGNAL(activated(int)), this, SLOT(currencySelectionChanged(int)));
        // Create the timer
        ...
        // Get the latest AITTCOIN release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void AITTCoinGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/aitttext")));
        m_toolbar->setMaximumWidth(65);
...
void AITTCoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool AITTCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool AITTCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void AITTCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AITTCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void AITTCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void AITTCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void AITTCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AITTCoinGUI::optionsClicked()
{
...
void AITTCoinGUI::showDebugWindow()
{
...
void AITTCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void AITTCoinGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void AITTCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void AITTCoinGUI::openClicked()
{
...
void AITTCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AITTCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AITTCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AITTCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AITTCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AITTCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** AITT START */
void AITTCoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void AITTCoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void AITTCoinGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void AITTCoinGUI::gotoRestrictedAssetsPage()
{
...
void AITTCoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to AITTCoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void AITTCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AITTCoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void AITTCoinGUI::updateHeadersSyncProgressLabel()
{
...
void AITTCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void AITTCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("AITTCoin"); // default title
...
void AITTCoinGUI::changeEvent(QEvent *e)
{
...
void AITTCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AITTCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AITTCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "AITT")
        msg += tr("Amount: %1\n").arg(AITTCoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(AITTCoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void AITTCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AITTCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AITTCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AITTCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AITTCoinGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void AITTCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AITTCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AITTCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AITTCoinGUI::detectShutdown()
{
...
void AITTCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void AITTCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void AITTCoinGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(AITTCoinGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void AITTCoinGUI::subscribeToCoreSignals()
{
...
void AITTCoinGUI::unsubscribeFromCoreSignals()
{
...
void AITTCoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void AITTCoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<AITTCoinUnits::Unit> units = AITTCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AITTCoinUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(AITTCoinUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(AITTCoinUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (AITTCoinUnits::Unit u : AITTCoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(AITTCoinUnits::name(u)), this);
...
{
    setText(AITTCoinUnits::name(newUnits));
}
...
void AITTCoinGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AITTCoinGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "AITTCoinGUI::onPriceUnitChange: " + QString::number(newIndex);
    ...
    this->comboAITTUnit->setCurrentIndex(newIndex);
    this->getPriceInfo();
}

void AITTCoinGUI::getPriceInfo()
{
...
void AITTCoinGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/AITTCoinProject/AITTCOIN/releases"));
