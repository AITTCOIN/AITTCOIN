...
#include "aittcoinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a AITTCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new AITTCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new AITTCoinAddressCheckValidator(parent));
}
...
bool parseAITTCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no aittcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("aittcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!AITTCoinUnits::parse(AITTCoinUnits::AITT, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseAITTCoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert aittcoin:// to aittcoin:
    //
    //    Cannot handle this later, because aittcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("aittcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "aittcoin:");
    }
    QUrl uriInstance(uri);
    return parseAITTCoinURI(uriInstance, out);
}

QString formatAITTCoinURI(const SendCoinsRecipient &info)
{
    QString ret = QString("aittcoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(AITTCoinUnits::format(AITTCoinUnits::AITT, info.amount, false, AITTCoinUnits::separatorNever));
        paramCount++;
    }
...
bool openAITTCoinConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(AITTCOIN_CONF_FILENAME);
    ...
    /* Open aittcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "AITTCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "AITTCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("AITTCoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for AITTCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "aittcoin.desktop";
    return GetAutostartDir() / strprintf("aittcoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a aittcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=AITTCoin\n";
        else
            optionFile << strprintf("Name=AITTCoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the aittcoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef aittcoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (aittcoinAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, aittcoinAppUrl);

    CFRelease(aittcoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add aittcoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, aittcoinAppUrl, nullptr, nullptr);
    }
