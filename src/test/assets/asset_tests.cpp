...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of AITTCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("AITT", type));
        BOOST_CHECK(!IsAssetNameValid("AITTCOIN", type));
        BOOST_CHECK(!IsAssetNameValid("AITTCOIN", type));

        //- Versions of AITTCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("AITTCOIN.COIN", type));
        BOOST_CHECK(IsAssetNameValid("AITTCOIN_COIN", type));
        BOOST_CHECK(IsAssetNameValid("AITTSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERAITT", type));
        BOOST_CHECK(IsAssetNameValid("AITTCOINSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEAITTCOIN", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_AITTCOINS", type));
        BOOST_CHECK(IsAssetNameValid("SEAITTOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("AITTCOIN", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("AITTCOIN", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
