...
// Block rewards
    ...
    CAmount minerSubsidy=nSubsidy;

    if(nHeight==1){
        minerSubsidy=nSubsidy-4752925000*COIN;//for dev  invest and community
        coinbaseTx.vout.resize(2);
        coinbaseTx.vout[0].scriptPubKey = scriptPubKeyIn;
        coinbaseTx.vout[0].nValue = nFees + minerSubsidy;
        //DEV Address
        std::string dev_address =GetParams().OrgAddress();;
        CTxDestination destDevAutonomous = DecodeDestination(dev_address);
        if (!IsValidDestination(destDevAutonomous)) {
            LogPrintf("IsValidDestination: Invalid AITT Coin DEV address %s \n", dev_address);
        }
        ...
        coinbaseTx.vout[1].nValue=4752925000*COIN;
    }
    else{
...
void static AITTCoinMiner(const CChainParams& chainparams)
{
    LogPrintf("AITTCoinMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aittcoin-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("AITTCoinMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("AITTCoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("AITTCoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("AITTCoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("AITTCoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateAITTCoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&AITTCoinMiner, boost::cref(chainparams)));
    }
