...
The RPM spec file provided here is for AITTCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://aittcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[AITTCoin git repository](https://github.com/AITTCoinProject/AITTCOIN) but are not part of
...
Currently only `Source10` is used, for the example `aittcoin.conf` file.
...
in the aittcoin git repository. At present only an SVG version of the AITTCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the aittcoin
...
needed when 0.12.1 is released, a proper fix is already in the AITTCoin git
master branch.
...
idea to build AITTCoin Core against a static version of that library compiled
...
The `aittcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `aittcoin-server` package. At this point, most RPM based
...
The `aittcoin-server` package should have SELinux support. How to properly do
...
AITTCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/aittcoin/). He has been
packaging AITTCoin for Fedora at least since 2012.
...
4. I renamed the `aittcoin` package that contains the Qt GUI to `aittcoin-core` as
that appears to be how the general population refers to it, in contrast to
`aittcoin-xt` or `aittcoin-classic`. I wanted to make sure the general population
