...
sudo find ${DISTNAME}/bin -type f -executable -exec ../AITTCOIN/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../AITTCOIN/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo aittcoin-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find aittcoin-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/AITTCOIN
...
cp -f aittcoin-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf AITTCOIN/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/AITTCOIN/aittcoin-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/aittcoin-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer aittcoin-*-win*-unsigned.tar.gz to ~/win64/  
...
cd ~/
sudo rm -rf win32
cp -rf AITTCOIN/contrib/windeploy ~/sign/win32
...
#transfer aittcoin-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
git clone https://github.com/AITTCOIN/AITTCOIN
mkdir ~/AITTCOIN/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder AITTCOIN/depends/SDKs
cd AITTCOIN/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/AITTCOIN
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "AITTCoin-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/aittcoin-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer aittcoin-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
