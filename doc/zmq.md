...
The AITTCoin Core daemon can be configured to act as a trusted "border
router", implementing the aittcoin wire protocol and relay, making
...
The ZeroMQ feature in AITTCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building aittcoind:
...
For instance:
    ...
    $ aittcoind -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/aittcoind.tx.raw
...
These options can also be provided in aittcoin.conf.
...
From the perspective of aittcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into aittcoind directly. Furthermore, no information is
...
using. AITTCoind appends an up-counting sequence number to each
