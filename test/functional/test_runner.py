...
# Copyright (c) 2017-2021 The AITTCoin Core developers
...
`test/functional/test_framework/test_framework.py:AITTCoinTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/aittcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and aittcoind
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_aittcoind = config["components"].getboolean("ENABLE_AITTCOIND")
    if not (enable_wallet and enable_cli and enable_aittcoind):
        print("No functional tests to run. Wallet, utils, and aittcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if aittcoind is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "aittcoind"]) is not None:
            print("%sWARNING!%s There is already a aittcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "AITTCOIND" not in os.environ:
        os.environ["AITTCOIND"] = build_dir + '/src/aittcoind' + exeext
        os.environ["AITTCOINCLI"] = build_dir + '/src/aittcoin-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `aittcoin-cli help` (`rpc_interface.txt`).
